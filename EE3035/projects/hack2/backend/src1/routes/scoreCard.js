...
const saveUser = async (name, subject, score) => {
  const existing = await Comment.findOne({ name, subject });
  if (existing) {
    try {
      await Comment.updateOne({ name, subject }, { name, subject, score });
    ...
    try {
      const newUser = new Comment({ name, subject, score });
      await newUser.save();
...
const queryUser = async (type, queryString) => {
  const query =
    type === "name"
      ? await Comment.find({ name: queryString })
      : await Comment.find({ subject: queryString });
  if (query.length > 0) {
    const x = [];
